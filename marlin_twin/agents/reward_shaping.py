...
class COLREGsRewardShaper:
    ...
    def compute_reward(
        ...
        for e in encounters:
            ...
            # Safety penalty for small CPA
            if e.cpa_distance < 100.0:
                r_safety -= 10.0  # Heavy collision risk
            elif e.cpa_distance < 500.0:
                r_safety -= np.exp(-e.cpa_distance / 200.0)

            # Rule 17 Stand-on Logic
            if e.encounter_type == EncounterType.CROSSING_STAND_ON:
                if e.tcpa > 60.0 and abs(action.rudder_angle) > 0.1:
                    r_colregs -= 2.0  # Premature deviation penalty
                elif e.tcpa <= 60.0 and abs(action.rudder_angle) >= 0.1:
                    r_colregs += 1.0  # Emergency evasive maneuver reward

        return float(w_safety * r_safety + w_colregs * r_colregs + w_efficiency * (state.speed / 15.0))
