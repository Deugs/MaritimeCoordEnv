...
class RolloutBuffer:
    ...
    def __init__(self, buffer_size: int = 500, n_agents: int = 5, obs_dim: int = 32, act_dim: int = 2):
        ...
        self.adv_buf = np.zeros((buffer_size, n_agents), dtype=np.float32)
        self.ret_buf = np.zeros((buffer_size, n_agents), dtype=np.float32)
        self.ptr = 0
    ...
    def compute_returns_and_advantages(self, last_values: np.ndarray, gamma: float = 0.99, gae_lambda: float = 0.95) -> None:
