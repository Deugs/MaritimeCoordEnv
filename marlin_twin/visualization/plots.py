# ============================================================================
# FILE: marlin_twin/visualization/plots.py
# ============================================================================

import matplotlib.pyplot as plt
import numpy as np
from marlin_twin.data_classes import (
    VoyageEpisode, CoordinationResilienceMetrics, MaritimeExperimentResult, MaritimeScene
)

class MaritimeVisualizer:
    """Generates publication-quality figures for paper submission."""

    def plot_trajectories(self, episode: VoyageEpisode | None = None, output_path: str | None = None) -> np.ndarray:
        fig, ax = plt.subplots(figsize=(8, 8))
        ax.set_title("Multi-Vessel Voyage Trajectories & Encounters", fontsize=12, fontweight='bold')
    ...
    def plot_resilience_curve(self, metrics: CoordinationResilienceMetrics, output_path: str | None = None) -> np.ndarray:
        ...
        ax.plot(levels, scores, 'o-', color='#1f77b4', linewidth=2.5, label="MARLIN-Twin (GAT + DT)")
    ...
    def plot_communication_heatmap(self, result: MaritimeExperimentResult | None = None, output_path: str | None = None) -> np.ndarray:
    ...
    def plot_encounter_graph(self, scene: MaritimeScene | None = None, output_path: str | None = None) -> np.ndarray:
        return self.plot_trajectories(output_path=output_path)

    def plot_colregs_compliance(self, result: MaritimeExperimentResult | None = None, output_path: str | None = None) -> np.ndarray:
