...
def render_fig1_system_architecture():
    ...
    rect1 = patches.FancyBboxPatch((0.02, 0.55), 0.28, 0.38, boxstyle="round,pad=0.02",
                                  facecolor=bg_blue, edgecolor='#1f77b4', linewidth=2)
    ax.add_patch(rect1)
    ax.text(0.16, 0.86, "Physical Environment\n& Vessel Dynamics", ha='center', va='center', fontweight='bold', color='#003366')
    ax.text(0.16, 0.70, "• 3-DOF MMG RK4 Solver\n• Non-linear Maneuvering\n• IMO Sea Trials (Turning/Zigzag)\n• CPA / TCPA Collision Engine", ha='center', va='center', fontsize=7.5)

    # Box 2: Digital Twin Fusion Layer
    rect2 = patches.FancyBboxPatch((0.36, 0.55), 0.28, 0.38, boxstyle="round,pad=0.02",
                                  facecolor=bg_green, edgecolor='#2ca02c', linewidth=2)
    ax.add_patch(rect2)
    ax.text(0.50, 0.86, "Digital Twin\nState Estimator", ha='center', va='center', fontweight='bold', color='#004d00')
    ax.text(0.50, 0.70, "• ITU-R M.1371 AIS Noise\n• 5x5 Extended Kalman Filter\n• JPDA Track Association\n• Blackout Dead Reckoning", ha='center', va='center', fontsize=7.5)

    # Box 3: Communication & Policy Layer
    rect3 = patches.FancyBboxPatch((0.70, 0.55), 0.28, 0.38, boxstyle="round,pad=0.02",
                                  facecolor=bg_orange, edgecolor='#ff7f0e', linewidth=2)
    ax.add_patch(rect3)
    ax.text(0.84, 0.86, "Graph Neural Network\n& Policy Layer", ha='center', va='center', fontweight='bold', color='#804000')
    ax.text(0.84, 0.70, "• PyG Graph Builder\n• Multi-Head GAT Encoder\n• Bandwidth Priority Queue\n• 128-bit Binary Telemetry", ha='center', va='center', fontsize=7.5)

    # Box 4: Multi-Agent RL & Action Execution
    rect4 = patches.FancyBboxPatch((0.20, 0.05), 0.60, 0.38, boxstyle="round,pad=0.02",
                                  facecolor=bg_purple, edgecolor='#9467bd', linewidth=2)
    ax.add_patch(rect4)
    ax.text(0.50, 0.36, "CTDE MAPPO Training Infrastructure", ha='center', va='center', fontweight='bold', color='#4b0082')
    ax.text(0.50, 0.20, "• Centralized Critic V(s_global) + Decentralized Actors pi_i(a_i|o_i)\n• GAE (gamma=0.99, lambda=0.95) Advantage Estimation\n• 2-Stage Curriculum: Spatial COLREGs Pre-training -> Comms Loss Resilience Fine-Tuning\n• Action Output: Propeller RPM, Rudder Angle, Message Priority", ha='center', va='center', fontsize=7.5)
    ...
    ax.set_title("Figure 1: MARLIN-Twin Overall System Architecture Block Diagram", fontweight='bold', pad=10)
    plt.tight_layout()
...
def render_fig2_digital_twin_flowchart():
    ...
    for (pos, text, col) in rects:
        box = patches.FancyBboxPatch(pos, 0.20, 0.32, boxstyle="round,pad=0.02",
                                    facecolor='#f9f9f9', edgecolor=col, linewidth=2)
        ax.add_patch(box)
        ax.text(pos[0]+0.10, pos[1]+0.16, text, ha='center', va='center', fontweight='bold', fontsize=8.0)

    # Decision diamond
    diamond = patches.Polygon([[0.46, 0.51], [0.49, 0.59], [0.52, 0.51], [0.49, 0.43]],
                              facecolor='#fff0f0', edgecolor='#d62728', linewidth=1.5)
    ...
    ax.set_title("Figure 2: Digital Twin EKF/JPDA State Estimation & Outage Recovery Flowchart", fontweight='bold', pad=10)
    plt.tight_layout()
...
def render_fig3_gat_attention_diagram():
    ...
    for n, (x, y) in pos.items():
        ...
        ax.add_patch(circle)
        ax.text(x, y, f"V{n}", ha='center', va='center', fontweight='bold', color='white' if n==0 else 'black')

    # Draw edge attention arrows
    edges = [(0, 1, r"$\alpha_{i1}=0.42$"), (0, 2, r"$\alpha_{i2}=0.35$"), (0, 3, r"$\alpha_{i3}=0.13$"), (0, 4, r"$\alpha_{i4}=0.10$")]
    for u, v, weight_str in edges:
        ...
        mid_x = (x1 + x2) / 2
        mid_y = (y1 + y2) / 2
        ax.text(mid_x, mid_y, weight_str, fontsize=8.0, ha='center', va='center', bbox=dict(boxstyle='round,pad=0.2', fc='white', ec='gray', alpha=0.9))

    ax.set_title("Figure 3: Multi-Head GAT Graph Attention Mechanism (Attention Weights alpha_ij)", fontweight='bold', pad=10)
    plt.tight_layout()
