...
Programmatically renders camera-ready system architecture block diagrams and
EKF/JPDA flowcharts at 300 DPI. These are conceptual, hand-drawn diagrams with
no real-run dependency; see scripts/generate_ieee_figures.py's
render_fig3_gat_attention_diagram() for the GAT attention figure, which is
computed from a real trained checkpoint rather than drawn by hand.
Usage:
...
def main():
    ...
    print("(Figure 3 -- GAT attention -- moved to generate_ieee_figures.py: it now computes")
    print(" real attention weights from a trained checkpoint rather than an illustration.)")
