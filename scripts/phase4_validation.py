...
def main():
    ...
    for lam in degradation_levels:
        ...
        for step in range(50):
            ...
            for vid, agent_obs in obs.items():
                act_vec = policies[vid].act(agent_obs.own_state.x / 5000.0 * np.ones(32, dtype=np.float32), deterministic=True)
        ...
        heatmap_matrix.append([mean_reward, lam * 9600.0, lam * 100.0, float(np.mean(delivered_counts))])
    ...
    # Degradation Curve
    rewards_list = [sweep_results[l] for l in degradation_levels]
    ax1.plot(degradation_levels, rewards_list, 'o-', color='#1f77b4', linewidth=2.5, label="MARLIN-Twin GAT")
