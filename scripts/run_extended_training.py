...
def main():
    ...
    for i in range(len(seeds)):
        smoothed_rewards[i] = np.convolve(all_seed_rewards[i], np.ones(window)/window, mode='same')
    ...
    ax.plot(episodes, mean_rewards, color='#1f77b4', linewidth=2.2, label="MARLIN-Twin MAPPO (Mean of 5 Seeds)")
    ax.fill_between(episodes, mean_rewards - std_rewards, mean_rewards + std_rewards, color='#1f77b4', alpha=0.25, label="±1 Std Dev (95% CI)")

    ax.axvline(stage1_cutoff, color='#d62728', linestyle='--', linewidth=1.5, label=f"Stage 2 Transition (Ep {stage1_cutoff})")
