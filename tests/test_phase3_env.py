...
import pytest
import numpy as np
from marlin_twin.data_classes import MaritimeExperimentConfig, VesselAction, EncounterType, COLREGsRule, Encounter
...
def test_colregs_rule17_reward_shaping():
    ...
    encounters = [
        Encounter(
            vessel_i=0, vessel_j=1,
            encounter_type=EncounterType.CROSSING_STAND_ON,
            colregs_rule=COLREGsRule.RULE_17_STAND_ON,
            cpa_distance=600.0, cpa_time=40.0, tcpa=40.0, dcpa=600.0, relative_bearing=0.2
        )
    ]
